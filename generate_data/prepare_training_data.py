...
def parse_args():
    ...
    parser.add_argument('--dataset_name', type=str, default='superclevr0719')
    ...
    parser.add_argument('--dataset_path', type=str, default='../data/SuperCLEVR_20220719')
...
def worker(split, save_path, args, start_idx=0, end_idx=500):
    ...
    for i, fname in enumerate(fnames):
        ...
        scene_file = os.path.join(scene_path, fname + '.json')
        mask_file = os.path.join(mask_path, fname + '.png')
        if not os.path.exists(img_file):
            print("Skipped -- No image: ", img_file)
            continue

        # annotation
        if args.part_mask:
            save_dict, masks = process_sample(fname, img_file, scene_file, mask_file, args)
        else:
            save_dict = process_sample(fname, img_file, scene_file, None, args)
        ...
        if args.background_path:
            ...
            obj_m = np.array(Image.open(mask_file))
            mask = np.where(obj_m[:, :, 0] == 64, 0, 1)
            mask = np.where(obj_m[:, :, 1] == 64, mask, 1)
            mask = np.where(obj_m[:, :, 2] == 64, mask, 1)

            bg_dir = os.path.join(args.background_path, args.categories, args.splits)
            bg_names = os.listdir(bg_dir)
            idx = np.random.randint(len(bg_names))
            bg = Image.open(os.path.join(bg_dir, bg_names[idx]))
            bg = np.array(bg.resize([img.shape[1], img.shape[0]]))

            img_w_bg = img.copy()
            for j in range(3):
                img_w_bg[:, :, j] = np.where(mask == 0, bg[:, :, j], img[:, :, j])

            img_w_bg = Image.fromarray(img_w_bg)
            img_w_bg.save(os.path.join(save_img_path, fname + '.png'))

    # if not adding background, then nothing changes for images
    if not args.background_path:
        os.system('mv {} {}'.format(img_path, save_img_path))
        os.system('ln -s {} {}'.format(save_img_path, img_path))

    # nothing changes for masks
    os.system('mv {} {}'.format(mask_path, save_mask_path))
    os.system('ln -s {} {}'.format(save_mask_path, mask_path))
