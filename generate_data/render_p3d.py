...
def parse_args():
    ...
    args.output_scene_file = os.path.join(args.output_dir, 'scenes.json')
...
def main():
    ...
    if args.distance is None:
        cat = args.categories[0].split('_')[0]
        if cat in ['bus', 'car', 'motorbike', 'aeroplane', 'bicycle', 'boat', 'diningtable', 'train']:
            args.distance = 6
        if cat in ['chair', 'bottle']:
            args.distance = 10
        if cat in ['sofa', 'tvmonitor']:
            args.distance = 8
    ...
    if args.clevr_scene_path is not None:
        ...
        clevr_scene = json.load(open(args.clevr_scene_path))
    ...
    if args.categories is not None:
        if args.categories == ['ALL']:
            info_pth = {k: obj_info['info_pth'][k] for k in obj_info['info_pth']}
        else:
...
def render_scene(args,
    ...
    while not (elevation_max >= elev >= elevation_min):
        elev = random.gauss(elevation_mean, elevation_variance)
