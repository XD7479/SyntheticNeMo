...
def parse_args():
    ...
    parser.add_argument('--save_exp_path', type=str)
    parser.add_argument('--prepare_for_pseudo', action='store_true')
...
def main():
    ...
    for i in tqdm(range(len(dataset))):
        ...
        # pred['azimuth'] = pred['azimuth']  # Xiaoding modified this

        if False:
            kp = sample['kp']
            kpvis = sample['kpvis']
            score_map = np.array(hmap(feature_map, memory).cpu())
            thr = np.linspace(-1, 1, 21)
            positive_pts = np.zeros((21, score_map.shape[0], score_map.shape[1], score_map.shape[2]))
            for i in range(len(thr)):
                positive_pts[i] = score_map > thr[i]
            TP = np.zeros((21, score_map.shape[0]))
            FP = np.zeros((21, score_map.shape[0]))
            TP_and_FN = np.zeros((21, score_map.shape[0]))
            FP_and_TN = np.zeros((21, score_map.shape[0]))
            for i in range(score_map.shape[0]):
                TP[:, i] = (positive_pts[:, i, int(kp[i][1]) // args.down_sample_rate,
                            int(kp[i][0]) // args.down_sample_rate] > 0) * kpvis[i]
                FP[:, i] = np.sum(positive_pts[:, i, :, :], axis=(1, 2)) - TP[:, i]
                TP_and_FN[:, i] = kpvis[i]
                FP_and_TN[:, i] = score_map.shape[1] * score_map.shape[2] - TP_and_FN[:, i]
            pred['TP'] = TP
            pred['FP'] = FP
            pred['TP_and_FN'] = TP_and_FN
            pred['FP_and_TN'] = FP_and_TN

        if False:
            kp = sample['kp']
            kpvis = sample['kpvis']
            score_map = np.array(hmap(feature_map, memory).cpu())
            x_ = score_map.shape[2]
            score_map = score_map.reshape(score_map.shape[0], -1)

            max_ = np.argmax(score_map, axis=1)

            score = np.zeros((score_map.shape[0], 2))
            max_idx = np.zeros((score_map.shape[0], 2))
            max_idx[:, 0] = max_ % x_
            max_idx[:, 1] = max_ // x_
            for i in range(score_map.shape[0]):
                score[i, 0] = score_map[i, max_[i]]
                if abs(int(kp[i][1]) // args.down_sample_rate - max_idx[i, 1]) < 2 and abs(
                        int(kp[i][0]) // args.down_sample_rate - max_idx[i, 0]) < 2 and kpvis[i]:
                    score[i, 1] = 1
            pred['kp_score'] = score
    ...
    os.makedirs(args.save_exp_path, exist_ok=True)
    save_path = os.path.join(args.save_exp_path, "{}_{}.npy".format(args.category, args.split))
    ...
    score = []
    if args.prepare_for_pseudo:
        for i in range(len(dataset)):
            sample = dataset[i]

            pred = all_pred[sample['img_name']]
        ...
        save_path = os.path.join(args.save_exp_path, f'{args.category}_{args.split}_100.txt')
